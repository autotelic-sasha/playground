...
namespace {{libname}} {
    ...
    void examples() {
        ...
        messages::message("Adding % to % gives %", 2, 2, {{libname}}_add(2, 2));
        messages::message("Adding % to % gives % (using a class this time)", 3, 3, {{libname}}_adder(3, 3).sum());
    }
    template< bool = true> // declaring it as a template is a way to work aroud c++ limitations about declaring things in headers
    void tests() {
        // code here will be run in test, examples and record mode
        AF_TEST_RESULT(4, {{libname}}_add(2, 2));
        AF_TEST_RESULT(6, {{libname}}_adder(3, 3).sum());
    }
}
